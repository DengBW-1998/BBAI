...
class AdversarialLearning(object):
    ...
    def forward_backward(self, real_data, netG_output, step, epoch, iter):
        ...
        ###########################
        # train with real
        #print(type(real)) #numpy
        #print(real.shape[0])
        label_real = torch.full((real.shape[0], 1), REAL_LABEL, device=self.device, requires_grad=False)
        label_fake = torch.full((real.shape[0], 1), FAKE_LABEL, device=self.device, requires_grad=False)
