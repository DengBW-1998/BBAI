...
class BaseMeta(Module):
    ...
    def __init__(self, nfeat, hidden_sizes, nclass, nnodes, dropout, train_iters, attack_features, lambda_, with_bias=False, lr=0.01, with_relu=False):
...
class Metattack(BaseMeta):
    ...
    def __init__(self, nfeat, hidden_sizes, nclass, nnodes, dropout, train_iters,
        ...
        super(Metattack, self).__init__(nfeat, hidden_sizes, nclass, nnodes, dropout, train_iters, attack_features, lambda_, with_bias=with_bias, with_relu=with_relu)
        ...
        output_weight = Parameter(torch.FloatTensor(previous_size, nclass))
        output_bias = Parameter(torch.FloatTensor(nclass))
    ...
    def get_meta_grad(self, features, adj_norm, idx_train, idx_unlabeled, labels, labels_self_training):
        ...
        print(f'GCN acc on unlabled data: {accuracy(output[idx_unlabeled], labels[idx_unlabeled]).item()}')
...
class MetaApprox(BaseMeta):
    ...
    def __init__(self, nfeat, hidden_sizes, nclass, nnodes, dropout, train_iters, attack_features, lambda_, with_relu=False, with_bias=False, lr=0.01):
        super(MetaApprox, self).__init__(nfeat, hidden_sizes, nclass, nnodes, dropout, train_iters, attack_features, lambda_, with_bias=with_bias, with_relu=with_relu)

        self.lr = lr
        self.adj_meta_grad = None
        self.features_meta_grad = None

        self.grad_sum = torch.zeros(nnodes, nnodes)
    ...
    def forward(self, features, ori_adj, labels, idx_train, idx_unlabeled, perturbations, ll_constraint=True, ll_cutoff=0.004):
        ...
        for i in tqdm(range(perturbations), desc="Perturbing graph"):
            ...
            row_idx, col_idx = unravel_index(adj_meta_approx_argmax, ori_adj.shape)
