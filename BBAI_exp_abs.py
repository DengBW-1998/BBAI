...
file_name="BBAI_exp_abs_"+dataset+"_"+str(ptb_rate)+"_"+train_model+".txt"
...
if not read_dir:
    for ite in range(iteration):    
        ...
        for _ in range(1):
            ...
            np.savetxt('./ptb_matrix/BBAI_exp_abs_ptb_'+dataset+'_'+str(ptb_rate)+'.dat',adj_matrix_flipped.copy().toarray(),fmt='%.2f',delimiter=' ')
            ...
            if ite==iteration-1:
                ...
                if dataset == 'dblp' or dataset == 'wiki':            
                    ...
                    print('BBAI_exp_abs auc:{:.5f}'.format(auc_score))
                    print('BBAI_exp_abs auc:{:.5f}'.format(auc_score), file=data_file)
                    
                if (dataset == 'cora' or dataset == 'citeseer' or dataset == 'pubmed'):
                    f1_scores_mean, _ = evaluate_embedding_node_classification(embedding, labels)
                    print('BBAI_exp_abs, F1: {:.5f} {:.5f}'.format(f1_scores_mean[0], f1_scores_mean[1]))
                    print('BBAI_exp_abs, F1: {:.5f} {:.5f}'.format(f1_scores_mean[0], f1_scores_mean[1]), file=data_file)

            adj=adj_matrix_flipped.copy()
            adj=standardize(adj)
            
else:
    adj_matrix_flipped=pd.read_csv('./ptb_matrix/BBAI_exp_abs_ptb_'+dataset+'_'+str(ptb_rate)+'.dat',sep=' ',header=None)
